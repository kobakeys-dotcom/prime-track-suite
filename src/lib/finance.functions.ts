...
export const getBudgetVsActual = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context, data }) => {
    ...
    let pcQ = sb.from("payment_claims").select("project_id, net_claim, status");
    ...
    (claims ?? []).forEach((c: any) => {
      ...
      cur.certified += Number(c.net_claim ?? 0);
...
export const calcPaymentClaim = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    return {
      ...
      gross_claim: +thisPeriodGross.toFixed(2),
      retention,
